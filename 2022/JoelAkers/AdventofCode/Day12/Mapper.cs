...
public class Mapper
{
  ...
  public Mapper(IReadOnlyList<string> fileLines)
  {
    ImportMap(fileLines);
  }
  ...
  private bool TryAddCoordinateToQueue(int x, int y, PathPoint currentPoint)
  {
    ...
    if (coordToAdd.Visited || coordToAdd.Height - currentCoord.Height > 1) 
      return false;
  ...
  private void ImportMap(IReadOnlyList<string> fileLines)
  {
    for (var i = 0; i < fileLines.Count; i++)
    {
      ...
      for (var j = 0; j < row.Length; j++)
      {
        ...
        {
          case 'S':
            _startingPosition = new Coord { Height = 'a', Visited = true, X = j, Y = i };
            nextRow.Add(_startingPosition);
            break;
          case 'E':
            _endPosition = new Coord { Height = 'z', Visited = false, X = j, Y = i };
            nextRow.Add(_endPosition);
            break;
          default:
  ...
  public void PrintMap(PathPoint finalPoint)
  {
    ...
    while (currentPoint.Coordinate != _startingPosition)
    {
      ...
      {
        map[currentPoint.Coordinate.Y][currentPoint.Coordinate.X] = 'S';
      }
